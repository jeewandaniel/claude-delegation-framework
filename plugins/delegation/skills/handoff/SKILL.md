---
name: handoff
description: Write the session handoff file so a later session can pick the work up. Use only on a request from the user for a handoff or to wrap up. Writes handoff.md in the project's Claude memory directory and reports the path.
---

# /handoff

Goal: the next session, starting on an empty context, continues without asking the user to repeat anything.

## Steps

1. Locate the memory directory. The system prompt names it ("persistent file-based memory at ..."). If the user named a path, use exactly that path. Create the directory if it is missing.
...
7. Reply to the user in exactly this shape and nothing more:
...
- If writing the file fails, report the exact error to the user and stop; do not claim it was saved.
